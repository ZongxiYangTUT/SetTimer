...
; Based on revision: 30d997321197c7c96d8e111e9ddd6c0ca8da5f09
...
ErrorTooManyFilesInDir=安装程序无法在目录“%1”中创建文件，因为里面包含太多文件。
...
SelectTasksLabel2=选择您想要安装程序执行哪些附加任务，然后点击“下一步”。
...
PreviousInstallNotCompleted=先前的程序安装或卸载未完成，需要您重启计算机以完成该安装。%n%n在重启计算机后，再次运行安装程序以完成 [name] 安装。
...
PrepareToInstallNeedsRestart=安装程序必须重启您的计算机。计算机重启后，请再次运行安装程序以完成 [name] 安装。%n%n要立即重启吗？
...
FinishedRestartLabel=为完成 [name] 安装，安装程序必须重新启动您的计算机。要立即重启吗？
FinishedRestartMessage=为完成 [name] 安装，安装程序必须重新启动您的计算机。%n%n要立即重启吗？
...
ErrorReplacingExistingFile=尝试替换下列文件时出错：
ErrorRestartReplace=重启替换文件失败：
